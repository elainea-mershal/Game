class Walter {
  ...
  void displayWalter() { //displays Walter
    ...
    else if (wasdKey==1&&idle) //if 'w' was the last key pressed and Walter is idle
    ...
    if (wPressed || aPressed || sPressed || dPressed) { //if 'w', 'a', 's', or 'd' is pressed
      if (k.kitchenCounter==1) //if the kitchen is displayed for the first time
        aCounter+=aSpeedW/2; //the speed of Walter's walking animations are halved
      else
