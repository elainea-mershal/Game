...
void roomTransition() { //allows transparency transitions between rooms
  ...
  if (transparency > transparencyMax) { //if the transparency is greater than the maximum transparency
    ...
    if (whiteTransition)
      roomCounter++;
    if (blackTransition) {
      if (displayOutside==false) { //if walter is moving outside from inside
      displayOutside=true; //display the outside area
      }
      else if (displayOutside) { //if walter is moving inside from outside
       displayOutside=false; //display the inside area
       lr.goingInside=true;
      }
    }
  }
