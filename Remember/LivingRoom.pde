class LivingRoom {
  ...
  void leave() { //allows Walter to leave the living room
    ...
    if (w.walterY>boundaryD) //if Walter is outside of the bottom living room boundary
      changeOutside=true; //the screen is to be changed to the outside

    if (w.walterY<boundaryU) //if Walter is outside of the upper living room boundary
      changeKitchen=true; //the screen is to be changed to the kitchen

    if ( w.walterX>hallBoundaryL && w.walterX<hallBoundaryR && w.walterY<hallBoundary) //if Walter is above the hall boundary
