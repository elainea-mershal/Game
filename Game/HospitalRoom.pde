class HospitalRoom { //<>//
  ...
  int doorTextCounter; //counter to display the door text
  int daTextCounter; //counter to display the angry door text
  ...
  int diaryTextCounter; //counter to display the diary text
  ...
  void diaryInteraction() { //allows Walter to interact with the diary
    if (displayDiaryText) { //if the diary text is to be displayed
      ...
      if (diaryTextCounter==1) { //if the diary text counter is 1
        diaryText.play(); //play the diary text gif
        noMove=true; //Walter cannot move
        image(diaryText, 360, 800); //display the diary text
        diaryTextCounter++; //increase the diary text counter by 1
      } else if (diaryTextCounter==2) { //otherwise, if the diary text counter is 2
        image(diaryText, 360, 800); //display the diary text
        noMove=true; //Walter cannot move
      } else if (diaryTextCounter==3) { //otherwise, if the diary text counter is 3
        noMove=false; //Walter can move
        displayDiaryText=false; //the diary text is no longer displayed
        diaryTextCounter=0; //the diary text counter is reset
      }
    }
  }
  ...
  void doorInteraction() { //allows Walter to interact with the door
    if (displayDoorText) { //if the door text is to be displayed
      if (doorTextCounter==1) { //if the door text counter is 1
        doorText.play(); //play the door text gif
        noMove=true; //Walter cannot move
        image(doorText, 360, 800); //display the door text
        doorTextCounter++; //increase the door text counter by 1
      } else if (doorTextCounter==2) { //otherwise, if the door text counter is 2
        image(doorText, 360, 800); //display the door text
        noMove=true; //Walter cannot move
      } else if (doorTextCounter==3) { //otherwise, if the door text counter is 3
        noMove=false; //Walter can move
        displayDoorText=false; //the door text is no longer displayed
        doorTextCounter=0; //the door text counter is reset
      }
      if (daTextCounter==7) { //if the angry door text counter is 7
        doorAText.play(); //play the angry door text gif
        noMove=true; //Walter cannot move
        image(doorAText, 360, 800); //display the angry door text
        doorTextCounter=0; //reset the door text counter
        daTextCounter++; //increase the angry door text counter by 1
      } else if (daTextCounter==8) { //otherwise, if the angry door text counter is 8
        image(doorAText, 360, 800); //display the angry door text
        noMove=true; //Walter cannot move
        doorTextCounter=0; //reset the door text counter
      } else if (daTextCounter==9) { //otherwise, if the angry door text counter is 9
        noMove=false; //Walter can move
  ...
  void hrKeyPressed() { //allows interactions when certain keys are pressed
    if (key=='e') { //if 'e' is pressed
      if (w.walterY<=doorY && w.walterX>=doorX && w.walterX<=doorX+doorW) { //if Walter is near the door
        ...
        doorTextCounter++; //the door text counter increases by 1
        daTextCounter++; //the angry door text counter increases by 1
      }
      if (w.walterX>dBoundaryL && w.walterX<dBoundaryL+100 && w.walterY>dBoundaryU && w.walterY<dBoundaryD || w.walterX<dBoundaryR && w.walterX>dBoundaryR-100 && w.walterY>dBoundaryU && w.walterY<dBoundaryD || w.walterY>dBoundaryU && w.walterY<dBoundaryU+70 && w.walterX>dBoundaryL && w.walterX<dBoundaryR || w.walterY<dBoundaryD && w.walterY>dBoundaryD-70 && w.walterX>dBoundaryL && w.walterX<dBoundaryR) { //if Walter is near the diary
        ...
        if (diaryTextCounter<2 || displayNo) //if the diary text counter is less than 2 and the arrow pointing towards no is displayed
          diaryTextCounter++; //the diary text counter increases by 1
        if (diaryTextCounter==2 && displayYes) //if the diary text counter is 2 and the arrow pointing towards yes is displayed
